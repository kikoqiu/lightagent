...
| `agent` | token 估算、压缩切分不孤立 tool 消息、触发判定、摘要合并、steering（消息行在并入上下文的那一刻广播：末轮流式中到达则本回合续跑一轮、收尾时才到则开跟进回合，事件顺序与历史顺序一致）、消息构建（含运行时环境行：模板不含 host 信息、agent.md 提示词同样追加）、重置、上下文用量（接口 `prompt_tokens` 锚定 + 追加估算、回合中实时广播） |
